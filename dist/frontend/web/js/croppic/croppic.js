...
(function (window, document) {
	...
	Croppic = function Croppic(id, options) {
	...
	Croppic.prototype = {
		...
		createImgUploadControls: function createImgUploadControls() {
			var that = this;
			...
			if (that.options.customUploadButtonId === '') {
				...
				that.imgUploadControl = $('#' + that.options.customUploadButtonId);that.imgUploadControl.show();
			}
		...
		bindImgUploadControl: function bindImgUploadControl() {
			...
			if (!$.isEmptyObject(that.croppedImg)) {
				...
				that.cropControlRemoveCroppedImage.on('click', function () {
					if (_typeof(that.options.onBeforeRemoveCroppedImg) === (typeof Function === 'undefined' ? 'undefined' : _typeof(Function))) {
					...
					if (_typeof(that.options.onAfterRemoveCroppedImg) === (typeof Function === 'undefined' ? 'undefined' : _typeof(Function))) {
		...
		loadExistingImage: function loadExistingImage() {
			var that = this;
		...
		afterUpload: function afterUpload(data) {
			...
			response = (typeof data === 'undefined' ? 'undefined' : _typeof(data)) == 'object' ? data : jQuery.parseJSON(data);
		...
		createEyecandy: function createEyecandy() {
			var that = this;

			that.imgEyecandy = that.img.clone();
			that.imgEyecandy.css({ 'z-index': '0', 'opacity': that.options.imgEyecandyOpacity }).appendTo(that.obj);
		},
		destroyEyecandy: function destroyEyecandy() {
			var that = this;
			that.imgEyecandy.remove();
		},
		initialScaleImg: function initialScaleImg() {
			var that = this;
		...
		createCropControls: function createCropControls() {
			var that = this;
				...
				that.img.css('z-index', 1000).on("mousemove touchmove", function (e) {
					...
					if (that.objH < that.imgH) {
						if (parseInt(that.img.css('top')) > 0) {
							that.img.css('top', 0);if (that.options.imgEyecandy) {
								that.imgEyecandy.css('top', 0);
							}
						}
						var maxTop = -(that.imgH - that.objH);if (parseInt(that.img.css('top')) < maxTop) {
							that.img.css('top', maxTop);if (that.options.imgEyecandy) {
						...
						var maxTop = that.objH - that.imgH;if (parseInt(that.img.css('top')) > maxTop) {
							that.img.css('top', maxTop);if (that.options.imgEyecandy) {
								that.imgEyecandy.css('top', maxTop);
							}
						}
					...
					if (that.objW < that.imgW) {
						if (parseInt(that.img.css('left')) > 0) {
							that.img.css('left', 0);if (that.options.imgEyecandy) {
								that.imgEyecandy.css('left', 0);
							}
						}
						var maxLeft = -(that.imgW - that.objW);if (parseInt(that.img.css('left')) < maxLeft) {
							that.img.css('left', maxLeft);if (that.options.imgEyecandy) {
						...
						var maxLeft = that.objW - that.imgW;if (parseInt(that.img.css('left')) > maxLeft) {
							that.img.css('left', maxLeft);if (that.options.imgEyecandy) {
								that.imgEyecandy.css('left', maxLeft);
							}
						}
		...
		zoom: function zoom(x) {
			...
			var maxTop = -(newHeight - that.objH);if (newTop < maxTop) {
				newTop = maxTop;
			}
			var maxLeft = -(newWidth - that.objW);if (newLeft < maxLeft) {
				newLeft = maxLeft;
			}
		...
		afterCrop: function afterCrop(data) {
			var that = this;
			try {
		...
		isAjaxUploadSupported: function isAjaxUploadSupported() {
		...
		CreateFallbackIframe: function CreateFallbackIframe() {
			var that = this;
			...
			if (!that.isAjaxUploadSupported()) {
				...
				var eventHandlermyFile = function eventHandlermyFile() {
					if (iframe.detachEvent) iframe.detachEvent("onload", eventHandlermyFile);else iframe.removeEventListener("load", eventHandlermyFile, false);
		...
		SubmitFallbackIframe: function SubmitFallbackIframe(that) {
			that.showLoader();
...
//# sourceMappingURL=croppic.js.map